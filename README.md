...
GitHub Pages で無料公開している場合に、押さえておくと安心な点です。

| 項目 | 内容 |
|------|------|
| **誰でも開けるか** | リポジトリが **Public（公共）** のとき、公開URLを知っていれば **ログインなしで閲覧できます**。URLを貼れば、知り合い・不特定多数が開ける状態です。 |
| **閲覧課金** | 一般的な **無料の GitHub Pages** では、「ページが1回開かれるたびにあなたに料金がかかる」仕組みではありません。知人が何度試しても、**閲覧回数ベースの請求が増えるイメージではない**と考えてよいです。 |
| **無料＝無制限ではない** | 転送量や利用規約に沿った **上限・禁止事項** はあります。通常の共有の範囲であれば問題になりにくいです。 |
| **仕様の変更** | GitHub の **料金・無料枠の内容は将来変わる可能性**があります。長く運用するときは、年に一度程度 [GitHub の料金・説明](https://github.com/pricing) を自分で確認すると確実です。 |
| **ソースが見える** | **Public リポジトリ** では、`index.html` や `fortunes.js` などの **コードが GitHub 上で世界中から閲覧可能**です。秘密にしたいロジックや文言がある場合は別途検討が必要です。 |
| **音声データ** | このアプリの仕様どおり（ブラウザ内だけで処理）であれば、**録音をサーバーに保存して転送量課金が発生する**、という心配は通常ありません。 |
